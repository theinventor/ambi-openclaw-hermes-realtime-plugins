...
export function install(host, options, identity, { runImpl = run } = {}) {
  ...
  const savedSettings = path.join(target, 'settings.local.json');
  const existing = fs.existsSync(savedSettings) ? JSON.parse(fs.readFileSync(savedSettings, 'utf8')) : {};
  if ((existing.expectedUser && existing.expectedUser !== identity.id) || (existing.expectedWorkspace && existing.expectedWorkspace !== identity.workspace_id)) throw new Error('Existing plugin belongs to another identity; use a separate runtime home');
  const settings = validateSettings({
    ...existing, configFile: path.resolve(options.config), stateFile: existing.stateFile ?? path.join(home, 'state/ambi-realtime/inbox.json'),
    expectedUser: identity.id, expectedWorkspace: identity.workspace_id, nodePath: process.execPath,
    notBefore: options.notBefore ?? existing.notBefore ?? null, sessionMode: options.sessionMode ?? existing.sessionMode ?? 'thread',
  });
  ...
  try {
    ...
    if (skillChanged && fs.existsSync(skill)) fs.rmSync(skill, { recursive: true, force: true });
