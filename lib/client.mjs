...
export class AmbiClient {
  ...
  run(args, { signal, timeout = this.settings.requestMs, onLine, normalTimeout = false, raw = false } = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      let stdout = '', stderr = '', pending = '', failure, expired = false, killTimer;
      const stop = () => {
  ...
  watch(onLine, signal) {
    return this.run(['notifications', 'watch', '--quiet-status'], {
      signal, onLine, timeout: this.settings.watchMs, normalTimeout: true,
    });
  }
}
