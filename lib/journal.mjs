...
export class Journal {
  ...
  enqueue(event, now = Date.now()) {
    ...
    try { this.save(); } catch (error) {
      if (existing) this.rows.set(id, existing); else this.rows.delete(id);
      throw error;
    }
    return true;
  }
  ...
  accept(id) {
    ...
    try { this.save(); } catch (error) { this.rows = before; throw error; }
  }
  ...
  fail(id) {
    ...
    this.rows.set(id, { ...row, attempts, availableAt: Date.now() + Math.min(this.settings.retryMs * 2 ** Math.min(attempts - 1, 6), 300_000) });
    try { this.save(); } catch (error) { this.rows.set(id, row); throw error; }
  }
