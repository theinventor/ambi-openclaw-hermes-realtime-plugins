...
export class EventPump {
  ...
  enqueue(event, source = 'unknown') {
    if (!this.running) return;
    try { if (!validateEvent(event, this.settings)) return; } catch (error) {
      this.error('event', error); return;
    }
    if (this.journal.enqueue(event)) this.log('queued', { id: event.notification_id, type: event.type, source });
  }
  ...
  async reconcile() {
    ...
    for (let pageIndex = 0; pageIndex < this.settings.maxPages; pageIndex++) {
      ...
      for (const event of page.events) this.enqueue(event, 'poll');
  ...
  async watchLoop() {
    ...
    while (this.running) {
      try {
        ...
        await this.client.watch(event => this.enqueue(event, 'watch'), this.controller.signal,
          message => this.log('transport', { message }));
  ...
  async handoff(id, event) {
    ...
    try {
      ...
      if (this.running) {
        try { this.journal.fail(id); } catch (journalError) {
          this.error('journal', journalError, id);
          await pause(this.settings.retryMs, this.controller.signal);
        }
        this.error('handoff', error, id);
      }
